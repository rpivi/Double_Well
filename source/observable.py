...
def mean_energy(trajectory, block_size, a=1.0, b=1.0):
    energies = jax.vmap(lambda x: V(x, a, b))(trajectory)
...
def heat_capacity(trajectory, T, block_size, a=1.0, b=1.0):
    energies = jax.vmap(lambda x: V(x, a, b))(trajectory)
...
def append_observables(results, trajectories, D, T, trajectory, acceptance_rate, a=1.0, b=1.0):
    ...
    E_mean, E_mean_err = mean_energy(trajectory, block_size, a, b)
    Cv, Cv_err = heat_capacity(trajectory, T, block_size, a,b)
